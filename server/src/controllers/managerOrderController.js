...
const createOrder = async (req, res) => {
    const { userId, quantity, total_Price, userName, email, phone, address, payment } = req.body;

    try {
        if (!userId || !quantity || !total_Price || !userName || !email || !phone || !address || !payment) {
...
const getOrderByStatus = async (req, res) => {
    try {
        ...
        const response = await managerOrderService.getOrderByStatus(statusOrder);
        return res.status(200).json(response);
    } catch (error) {
        return res.status(500).json({
            err: -1,
...
const getOrderByUserStatus = async (req, res) => {
    try {
        ...
        const response = await managerOrderService.getOrderByUserStatus(userId, statusOrder);
        return res.status(200).json(response);
    } catch (error) {
        return res.status(500).json({
            err: -1,
...
const updateStatusOrder = async (req, res) => {
    try {
        const orderId = req.params.orderId;
        const response = await managerOrderService.updateStatusOrder(req.body, orderId);
        return res.status(200).json(response);
    } catch (error) {
        return res.status(500).json({
            err: -1,
...
const updateOrderByUser = async (req, res) => {
    const { quantity, total_Price, userName, email, phone, addres, payment } = req.body;

    try {
        if (!quantity || !total_Price || !userName || !email || !phone || !addres || !payment) {
            return res.status(400).json({
                err: 1,
                msg: 'Missing inputs !',
            });
        ...
        const orderId = req.params.orderId;
        const reponse = await managerOrderService.updateOrderByUser(req.body, orderId);
        return res.status(200).json(reponse);
    } catch (error) {
        return res.status(500).json({
            err: -1,
...
const cancelOrder = async (req, res) => {
    try {
        const orderId = req.params.orderId;
        const response = await managerOrderService.cancelOrder(req.body, orderId);
        return res.status(200).json(response);
    } catch (error) {
        return res.status(500).json({
            err: -1,
